// exp05-ivf-512: IVF with C=512 centroids (tighter clusters → better recall).
//
// Same API as exp04. Only the embedded index changes: C=512 k-means centroids
// vs C=256 in exp04. With nprobe=16 the number of vectors searched is the same
// as exp04 nprobe=8, but cluster boundaries are tighter → fewer missed neighbors.
package main
...
func main() {
	...
	go func() {
		log.Printf("opening IVF index %s (nprobe=%d)", indexPath, nprobe)
