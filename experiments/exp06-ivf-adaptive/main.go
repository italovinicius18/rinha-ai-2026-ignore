// exp06-ivf-adaptive: two-pass adaptive IVF search on a C=512 index.
//
// Fast path (NPROBE, default 16): handles ~95.7% of queries (0 or 5 frauds out of 5).
// Fallback (IVF brute force, all 512 clusters): only for nFraud==2 or 3 (~4.3%).
// At ~4ms per fallback query, CPU budget impact is ~17% of the 0.45-CPU allotment.
package main
...
func (s *server) handleFraudScore(w http.ResponseWriter, r *http.Request) {
	...
	nFraud := s.ix.SearchKNNAdaptive(&q, s.nprobe)
...
func main() {
	...
	nprobe := parseEnvInt("NPROBE", defaultNprobe)
	...
	go func() {
		log.Printf("opening IVF index %s (nprobe=%d, adaptive brute-force fallback)", indexPath, nprobe)
